...
export const Donates = (propos) => {
  ...
  const depositErc20ToPortal = async (token, amount) => {
    try {
      if (rollups && provider) {
  ...
  const depositEtherToPortal = async (amount) => {
    try {
      if (rollups && provider) {
  ...
  const withdrawErc20 = async (amount, address) => {
    try {
      if (rollups && provider) {
  ...
  const withdrawErc721 = async (address, id) => {
    try {
      if (rollups && provider) {
